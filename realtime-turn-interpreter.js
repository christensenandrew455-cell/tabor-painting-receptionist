...
export function buildRealtimeTurnPrompt({ core, transcript, currentQuestionId, lead, history = [] }) {
  ...
  return [
    ...
    'Set every field not explicitly stated or corrected in the latest transcript to null.',
    ...
    'Use thanks_name only when currentQuestionId is name and a valid name was supplied.',
    'Use sorry for an unpleasant problem, sounds_good for a normal positive transition, thanks after information, got_it after a simple answer, or none.',
    'Use action summary_confirm or summary_correction only when currentQuestionId is confirm_summary.',
    'At ask_estimate or continue_estimate, a clear yes must be action yes and a clear no must be action no.',
    'At more_questions, a clear no must be more_questions_no and a clear yes must be more_questions_yes.',
    ...
    'At final confirmation, yes with no correction is summary_confirm. Yes but, actually, except, or any correction is summary_correction and must include only corrected fields.',
