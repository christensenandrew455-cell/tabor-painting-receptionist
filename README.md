...
Incoming audio uses far-field noise reduction and high-eagerness semantic voice activity detection. OpenAI does not automatically create or interrupt responses: caller turns are analyzed by the server-controlled flow. Short acknowledgements such as “okay” or “yeah” do not cut off the receptionist. An explicit interruption such as “wait,” “scratch that,” or “I meant…” cancels the current OpenAI response and clears Telnyx's queued playback so the correction can be handled immediately. Other speech captured while the receptionist is talking is queued and handled after that response finishes.
