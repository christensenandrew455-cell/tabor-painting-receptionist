...
const RETRY_QUESTIONS = Object.freeze({
  ...
  preferredDateOrDay: 'What date or upcoming day would you prefer?',
...
export function rewriteThinkingCueRequest(body = {}) {
  if (!clean(body.command_id).startsWith('thinking-cue-')) return body;
...
globalThis.fetch = async function intakeWordingFetch(input, init = {}) {
  const rawUrl = typeof input === 'string' || input instanceof URL ? String(input) : clean(input?.url);
  if (!/\/actions\/speak(?:\?|$)/i.test(rawUrl) || !init?.body) return originalFetch(input, init);

  try {
    const body = JSON.parse(String(init.body));
    const rewritten = rewriteThinkingCueRequest(body);
