...
test('only gpt-realtime-2.1-mini is configured as an AI model', () => {
  assert.match(models, /gpt-realtime-2\.1-mini/);
  assert.doesNotMatch(models, /['"]gpt-realtime-mini['"]/);
...
test('Realtime 2.1 mini listens natively without a transcription model', () => {
  ...
  assert.match(voice, /isAssistantLikeTranscript/);
  assert.match(voice, /never invent a greeting/i);
});
...
test('caller-facing speech is generated by the same Realtime session', () => {
  ...
  assert.match(voice, /Realtime speech wording drift/);
  assert.doesNotMatch(voice, /REALTIME_SPEECH_MISMATCH/);
  assert.doesNotMatch(voice, /did not match the requested phrase/);
  assert.match(controller, /engine: 'realtime'/);
  assert.doesNotMatch(controller, /engine: 'speech-api'/);
});

test('speech requests receive enough audio tokens and preserve usable output', () => {
  assert.match(voice, /max_output_tokens: 512/);
  assert.doesNotMatch(voice, /Math\.ceil\(request\.text\.length/);
  assert.match(voice, /event\.response\?\.status_details/);
  assert.match(voice, /Realtime speech incomplete but usable/);
  assert.match(voice, /speechRequest\.resolve\(splitPcmuFrames\(pcmu\)\)/);
});
