...
export function isAllowedOverlapInterruption(value = '') {
  return isStrictMeaningfulTranscript(value, MIN_MEANINGFUL_AUDIO_MS);
}
...
export function applyStrictSessionSettings(message = {}) {
  ...
  turnDetection.silence_duration_ms = Math.max(Number(turnDetection.silence_duration_ms || 0), STRICT_SILENCE_DURATION_MS);
...
function syntheticCancelled(socket, reason) {
  queueMicrotask(() => {
    originalEmit.call(socket, 'message', Buffer.from(JSON.stringify({ type: 'response.cancelled', reason })), false);
  });
}
...
WebSocket.prototype.send = function strictAudioTurnSend(data, ...args) {
  ...
  if (message.type === 'session.update') {
    ...
    console.log('[Strict audio turn guard]', {
      ...
      assistantInterruption: 'meaningful caller speech is preserved',
    });
...
WebSocket.prototype.emit = function strictAudioTurnEmit(eventName, ...args) {
  ...
  if (message.type === 'input_audio_buffer.speech_stopped') {
    ...
    if (itemId && state.speechStartedDuringAssistant) state.overlapItems.set(itemId, durationMs);
  ...
  if (message.type === 'conversation.item.input_audio_transcription.completed') {
    ...
    const startedDuringAssistant = state.overlapItems.has(itemId);
    ...
    if (startedDuringAssistant) {
      console.log('[Strict audio turn guard]', {
        action: 'preserved meaningful caller speech over receptionist',
        transcript,
        durationMs,
      });
    }
  ...
  if (state.forbiddenResponse && (message.type === 'response.audio.delta' || message.type === 'response.output_audio.delta')) {
    return false;
  }
