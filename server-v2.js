...
const TRANSCRIPT_WAIT_MS = Number(process.env.TRANSCRIPT_WAIT_MS || 1800);
const SILENCE_DURATION_MS = Number(process.env.VAD_SILENCE_DURATION_MS || 900);
const AUDIO_SPEED = Math.min(1.5, Math.max(0.25, Number(process.env.OPENAI_REALTIME_SPEED || 0.88)));
        ...
        audio: {
          input: {
            ...
            transcription: {
              ...
              language: 'en',
              prompt: 'Natural phone calls about painting estimates, names, email addresses, Massachusetts towns, dates, and times.',
            },
            turn_detection: {
              type: 'server_vad',
              threshold: 0.7,
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`Codec: PCMU 8 kHz; speech speed: ${AUDIO_SPEED}`);
});
