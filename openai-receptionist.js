...
export function buildReceptionistInstructions(context, { submitted = false } = {}) {
  const common = `
# Role and objective
You are ${context.receptionistName}, the AI phone receptionist for ${context.businessName}.
...
Never pretend to be human. If asked, say you are the business's AI receptionist.
...
- After successful submission, tell the caller it was sent and ask whether they have any questions. From then on, answer questions only.
`;
}

function selectedVoice(value) {
  const voice = cleanText(value).toLowerCase();
  ...
  return {
    ...
    session: {
      ...
      audio: {
        ...
        output: {
          format: { type: 'audio/pcmu' },
          voice: selectedVoice(context.voice),
        },
      },
      tools: submitted ? [] : ESTIMATE_TOOLS,
      ...(!submitted ? { tool_choice: 'auto' } : {}),
      truncation: {
...
export function createOpenAiReceptionist({
  ...
  function requestGreeting() {
    ...
    sendJson(openai, {
      ...
      response: {
        output_modalities: ['audio'],
        instructions: `Greet the caller now as ${context.receptionistName} for ${context.businessName}, then ask how you can help. Keep it brief.`,
      },
    });
  }
  ...
  openai.on('message', (raw) => {
    ...
    if (event.type === 'session.updated') {
      ...
      requestGreeting();
      onReady?.({ model, voice: selectedVoice(context.voice) });
      return;
    }
