...
patch(
  'opening',
  /function promptForOpening\(\) \{[\s\S]*?\n\}\n\nfunction realtimeInstructions/,
  "function promptForOpening() {\n  return `Hi, my name is Alex. I am the ${BUSINESS_NAME} receptionist. Would you like to set up an appointment for an estimate?`;\n}\n\nfunction realtimeInstructions"
);
...
patch(
  ...
  'Respond after about 1.5 seconds of caller silence. Do not jump in during a normal thinking pause. If the caller trails off or makes a side comment, wait for a clear answer or ask a short clarification.'
);

patch(
  'privacy and checklist',
  'SCRIPT RULE\nFollow the script about 90% verbatim.',
  'SCRIPT RULE\nNever guess a name from an email address, phone number, or caller ID. Ask for the name directly and only use the name they gave you. Before confirming or ending the call, make sure you have name, email, service, preferred day, preferred time, and best contact method. If any of those are missing, ask for the missing detail first. Follow the script about 90% verbatim.'
);
