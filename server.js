...
const RECORDING_MAX_LENGTH = process.env.RECORDING_MAX_LENGTH || '12';
const RECORDING_TIMEOUT = process.env.RECORDING_TIMEOUT || '3';
...
function getOpenAIClient() {
  if (!process.env.OPENAI_API_KEY) throw new Error('OPENAI_API_KEY is missing. Railway is not passing it to this running service.');
  return new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
}

function getResendClient() {
  if (!process.env.RESEND_API_KEY) throw new Error('RESEND_API_KEY is missing. Railway is not passing it to this running service.');
...
const businessProfile = {
  ...
  services: process.env.BUSINESS_SERVICES || 'interior painting, exterior painting, cabinet painting, drywall patching, staining, and small paint repairs',
  ...
  bookingRule: process.env.BOOKING_RULE || 'Collect the caller name, phone number, service needed, location, and preferred time. Do not promise an exact appointment. Say the owner will follow up to confirm.'
};
...
function getCall(callSid) {
  if (!calls.has(callSid)) {
    calls.set(callSid, {
      messages: [],
      lead: { name: '', phone: '', service: '', location: '', preferredTime: '', summary: '', complete: false },
      emailed: false
    });
...
async function getAiResponse(callSid, callState, userText, callerNumber) {
  ...
  const messages = [
    ...
    ...callState.messages,
    { role: 'user', content: `Caller phone number from ${PHONE_PROVIDER}: ${callerNumber || 'unknown'}\nCaller said: ${userText}` }
  ];

  logStep(callSid, 'Sending transcript to OpenAI chat', { model: process.env.OPENAI_MODEL || 'gpt-4o-mini', transcript: userText, messageCount: messages.length });
  ...
  callState.lead = { ...callState.lead, ...parsed.lead };
  if (!callState.lead.phone && callerNumber) callState.lead.phone = callerNumber;

  logStep(callSid, 'Updated lead state', { reply: parsed.reply, complete: callState.lead.complete, shouldEndCall: parsed.shouldEndCall, lead: JSON.stringify(callState.lead) });
  return parsed;
}

async function sendLeadEmail(callSid, callState) {
  if (!OWNER_EMAIL) return logStep(callSid, 'Skipping lead email because OWNER_EMAIL is missing');
  if (callState.emailed) return logStep(callSid, 'Skipping lead email because it was already sent');
...
function getCallSid(req) {
  return getRequestValue(req, 'CallSid', 'call_sid', 'callSid', 'call_control_id', 'call_id') || `local-test-call-${Date.now()}`;
}
...
function getRecordingDuration(req) {
  return getRequestValue(req, 'RecordingDuration', 'recording_duration', 'recordingDuration') || '';
}

function getRecordingStatus(req) {
  return getRequestValue(req, 'RecordingStatus', 'recording_status', 'recordingStatus') || '';
}
...
async function downloadRecordingToTempFile(callSid, recordingUrl) {
  ...
  if (!response.ok) throw new Error(`Could not download recording. HTTP ${response.status}`);
...
async function transcribeWithOpenAI(callSid, audioFilePath) {
  ...
  const transcription = await withTimeout(
    openai.audio.transcriptions.create({ file: fs.createReadStream(audioFilePath), model: OPENAI_TRANSCRIPTION_MODEL }),
...
app.all(TELNYX_RECORDING_WEBHOOK_PATH, async (req, res) => {
  ...
  const recordingDuration = getRecordingDuration(req);
  const recordingStatus = getRecordingStatus(req);
  const callState = getCall(callSid);
  let tempAudioPath = '';
...
app.all(TELNYX_STATUS_WEBHOOK_PATH, async (req, res) => {
  const callSid = getCallSid(req);
  const callStatus = String(getRequestValue(req, 'CallStatus', 'call_status', 'callStatus', 'status', 'event_type') || '').toLowerCase();
