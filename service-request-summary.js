...
export function summaryInput({ draft = {}, source = {}, demo = false } = {}) {
  return {
    ...
    structured_fields_read_separately: {
      ...
      request_urgency: cleanText(draft.requestUrgency),
...
export function buildRealtimeSummaryInstructions({ draft, source, demo = false } = {}) {
  return [
    ...
    'Do not include the caller name, service address, preferred day/time window, emergency/ASAP marker, consent, or confirmation. Do not add a diagnosis, solution, object, action, or fact the caller did not provide.',
    ...
    input: [
      {
        ...
        content: [
          ...
          'Do not include the caller name, service address, preferred day or time window, emergency/ASAP marker, consent, or confirmation in notes_summary because the server reads those separately.',
